...
def metrics(y_true, y_pred, category, category_dict):
    ...
    for c, res in res_by_category.items():
        ...
        metrics_by_category[c] = {
            'precision': round(precision_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro'), 4),
            'recall': round(recall_score(res['y_true'], np.around(np.array(res['y_pred'])).astype(int), average='macro'), 4),
